...
public:
    ...
    DllExport Big& operator*=(int i)  { premult(fn, i, fn); return *this; }
    DllExport Big& operator*=(const Big& b) { multiply(fn, b.fn, fn); return *this; }

    DllExport Big& operator/=(int i)  { subdiv(fn, i, fn);    return *this; }
    DllExport Big& operator/=(const Big& b) { divide(fn, b.fn, fn); return *this; }

    DllExport Big& operator%=(int i)  { convert(subdiv(fn, i, fn), fn); return *this; }
    DllExport Big& operator%=(const Big& b) { divide(fn, b.fn, b.fn); return *this; }
    ...
    DllExport friend Big operator-(const Big&);

    DllExport friend Big operator+(const Big&, int);
    DllExport friend Big operator+(int, const Big&);
    DllExport friend Big operator+(const Big&, const Big&);
    ...
    DllExport friend int operator%(const Big&, int);
    DllExport friend Big operator%(const Big&, const Big&);

    DllExport friend Big operator<<(const Big&, int);
    DllExport friend Big operator>>(const Big&, int);

    DllExport friend BOOL operator<=(const Big& b1, const Big& b2)
...
#endif
    DllExport friend Big pow(const Big&, int);               // x^m
    DllExport friend Big pow(const Big&, int, const Big&);  // x^m mod n
    DllExport friend Big pow(int, const Big&, const Big&);  // x^m mod n
    DllExport friend Big pow(const Big&, const Big&, const Big&);  // x^m mod n
    DllExport friend Big pow(const Big&, const Big&, const Big&, const Big&, const Big&);
                                                         // x^m.y^k mod n 
...
/* Montgomery stuff */
    ...
    DllExport friend Big nres(const Big&);
    DllExport friend Big redc(const Big&);
/*
...
*/       
    DllExport friend Big shift(const Big&b, int n);
    DllExport friend int length(const Big&b);
 ...
 * included in the text, unlike C I/O which does include CR. */
    ...
    DllExport friend istream& operator>>(istream&, Big&);
    DllExport friend ostream& operator<<(ostream&, const Big&);
    DllExport friend ostream& otfloat(ostream&, const Big&, int);

#endif

// output Big to a String
    DllExport friend char * operator<<(char * s, const Big&);
